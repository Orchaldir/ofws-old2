...
class AStarAlgorithmTest {
    ...
    inner class GetValues {
        ...
        private val index = TileIndex(0)
        private val node0 = AStarNode(index, 10, 100, null)
        private val node1 = AStarNode(index, 20, 200, null)
        private val algorithm = AStarAlgorithm(arrayOf(node0, null, node1))

        @Test
        fun `Get costs`() {
            assertEquals(listOf(10, null, 20), algorithm.getCostsToNode())
        }

        @Test
        fun `Get heuristics`() {
            assertEquals(listOf(100, null, 200), algorithm.getHeuristics())
        }
    }
