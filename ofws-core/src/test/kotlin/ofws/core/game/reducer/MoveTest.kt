...
import ofws.math.Direction.SOUTH
...
class MoveTest {
    ...
    fun `Move simple footprint`() {
        test(mapOf(index1 to entity), SimpleFootprint(index1), mapOf(index3 to entity), SimpleFootprint(index3, SOUTH))
    }
    ...
    fun `Move snake footprint`() {
        test(
            ...
            SnakeFootprint(listOf(index2, index0, index1), SOUTH)
        )
    }
    ...
    private fun test(
        ...
        val (newState, actions) = MOVE_REDUCER(state, Move(entity, SOUTH))
